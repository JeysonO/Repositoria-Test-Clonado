pipeline{
    ...
    stages{
        ...
        stage("Build Docker Image"){
            steps{
                script {
                    sh "/var/jenkins_home/scripts/amsac-tramite-service/prod/build_image.sh"
                }                
            }
        }
            ...
            steps{
                script {
                    withCredentials([string(credentialsId: 'docker-hub-pwd', variable: 'dockerHubPassword')]) {
                        sh "/var/jenkins_home/scripts/amsac-tramite-service/prod/push_image.sh"
                    }
                }                
            }
        }
                ...
                script {
                    withCredentials([string(credentialsId: 'gitlab-password', variable: 'githubServerPwd')]) {
                        sshagent(['test-server-amsac']) {
                            sh "/var/jenkins_home/scripts/amsac-tramite-service/prod/create_container.sh"
                        }
                    }
                }                
            }
